//
//  DetalhesSugeridosViewController.swift
//  DesafioJera
//
...
class DetalhesSugeridosViewController: UIViewController {
    ...
    @IBAction func paraAssistir(_ sender: Any) {
        ...
        if let perfilAtual = PerfisViewController.GlobalVariable.perfilAtul{
            ...
            if let idUsuarioAtual = PerfisViewController.GlobalVariable.idAtual{
                ...
                if let idFilme = self.idFilme{
                    ...
                    self.firestore.collection("usuarios").document(idUsuarioAtual).collection("Perfis").document(perfil).updateData(["FilmesParaAssistir":FieldValue.arrayUnion([idFilme])])
                    ...
                    if let nome = self.nome{
                        let alerta = alertas.alertas(titulo: "Filme Adicionado na Lista Para Assistir", erro: "Adicionado o filme: \(nome)")
